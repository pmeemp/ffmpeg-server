...
//  const longest = lines.reduce((m, s) => Math.max(m, s.length), 0);
  ...
  // longest ~18–22 => ~78–70
  // longest ~26–30 => ~66–60
//  const size = Math.round(86 - (longest * 0.95));

// cap if longest line is very short (prevents huge captions)
...
module.exports = function t2(vars = {}) {
  const productName = String(vars.productName || "").trim();

  // Base sentence (same as your examples)
  const raw =
    `TikTok bullied the price ` +
    `down and now ${productName} ` +
    `is on a massive sale ` +
